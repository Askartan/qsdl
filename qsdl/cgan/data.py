...
class ReconstructionDataset(Dataset):
    def __init__(self, h5_path, indices=None):
        with hdf.File(f"{h5_path}", "r") as f:
            self.wigners = np.asarray(f["wigner"][:])           #(N_samples, 128, 128)
            self.rhos_clean = np.asarray(f["rhos_clean"][:])    #(N_samples, 128, 128)

            if indices is not None:
                self.wigners = self.wigners[indices]
                self.rhos_clean = self.rhos_clean[indices]

    def __len__(self):
        return len(self.wigners)

    def __getitem__(self, i: int):
        w = torch.from_numpy(self.wigners[i]).float().unsqueeze(0) # (1,128,128) do pytorcha
